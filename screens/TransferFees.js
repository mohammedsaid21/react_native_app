import { View, Text, StyleSheet, TextInput, FlatList, Animated, TouchableOpacity } from 'react-native'
...
import TransferFeeHelperUtil from "../helper/TransferFeeHelperUtil"
import { Ionicons } from '@expo/vector-icons';
import HelperUtil from '../helper/HelperUtil'
import AddNewTransferFees from '../components/modals/AddNewTransferFees'
...
const TransferFees = () => {
  ...
  useEffect(() => {
    totalPages = Math.ceil(transferFees.length / ITEM_PER_PAGE)
  }, [page,])
  ...
  const renderItem = ({ item, index }) => (
    <View style={[styles.row, expandedIndex === index ? styles.borderActive : ""]} key={`${item.currency}-${index}`}>
      ...
      <TouchableOpacity onPress={() => toggleExpanded(index)} style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        <View style={{ flex: 1.3, }}>
          <Text style={{ textAlign: 'center' }}>{item.fromCountry}</Text>
        </View>
        <View style={{ flex: 1.3, }}>
          <Text style={{ textAlign: 'center' }}>{item.toCountry}</Text>
        </View>
        <View style={{ flex: 1.3, flexDirection: 'row', justifyContent: 'center' }}>
          <Text style={{ textAlign: 'center' }}>{item.fee}</Text>
          ...
          <View style={styles.expandedView}>
            ...
            <View style={[styles.expandedViewFlex]}>
              <Text style={styles.expandedViewText}>
                {i18n.t('transfer_fee_condition')}:
              </Text>
              <Text style={styles.expandedValue}>
                {TransferFeeHelperUtil.getTransferFeeInWrittenPatternLan(item.feeDelm, item.rangeValue,
                  item.rangeValueMin, item.rangeValueMax)
                }
              </Text>
            </View>
            ...
            <View style={styles.expandedViewFlex}>
              <Text style={styles.expandedViewText}>
                {i18n.t('transfer_fee_type')}:
              </Text>
              <Text style={styles.expandedValue}>
                {HelperUtil.getFeeTypeNameLang(item.feeType)}
              </Text>
            </View>
            ...
            <View style={[styles.expandedViewFlex]}>
              <IconWithLabel iconName="cog-outline" label={i18n.t('user_transfer_fee_update')} onPress={() => showUpdateModal(item.id)}
              // disabledClick={item.numberOfAccountTransactions === 0 ? false : true} 
              />
              <IconWithLabel iconName="trash-outline" label={i18n.t("curd_general_delete")} disabledClick={true} />
            </View>
    ...
    <>
      {
        page === 0 ?
          <View style={styles.paginationContainer}>
            <Text style={styles.paginationText}>{`${i18n.t("pagination_page_label")} ${page + 1} ${i18n.t("pagination_of_label")} ${Math.ceil(transferFees.length / ITEM_PER_PAGE)}`}</Text>
          </View>
          :
          searchLess10 ?
            ...
            <View style={styles.paginationContainer}>
              ...
              <Text style={styles.paginationText}>{`${i18n.t("pagination_page_label")} ${page + 1} ${i18n.t("pagination_of_label")} ${Math.ceil(transferFees.length / ITEM_PER_PAGE)}`}</Text>
              <TouchableOpacity
                disabled={page >= Math.ceil(transferFees.length / ITEM_PER_PAGE) - 1}
                onPress={handleNext}
                style={[styles.paginationButton, page >= Math.ceil(transferFees.length / ITEM_PER_PAGE) - 1 && styles.disabled]}>
                {/* <Text style={styles.paginationButtonText}>{'>'}</Text> */}
  ...
  let dataToShow = transferFees?.slice(page * ITEM_PER_PAGE, (page + 1) * ITEM_PER_PAGE)
  ...
  const handleSearch = (query) => {
    ...
    if (queryLower.length === 0) {
      ...
      setFilterData(transferFees?.slice(page * ITEM_PER_PAGE, (page + 1) * ITEM_PER_PAGE))
    }

    setFilterData(dataToShow.filter((user) =>
      user.fromCountry.includes(queryLower)
    ))
  ...
  return (
    <View style={styles.container} ref={scrollUp}>
      ...
      <FlatList
        ListHeaderComponent={
          ...
          <Button title='Print' /> */}
                ...
                <IconWithLabel iconName="add-circle-outline" label={i18n.t("user_transfer_fee_add_new")} onPress={() => showAddModal()} />
              </View>
            </View>
      ...
      <AddNewTransferFees
        showAddPriceModal={showAddPriceModal} setShowAddPriceModal={setShowAddPriceModal}
        id={id} setId={setId}
      />
